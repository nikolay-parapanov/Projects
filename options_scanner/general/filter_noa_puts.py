...
def filter_noa_puts_details():

    # Define file paths
    raw_file = 'noa/comprehensive_pivot.csv'
    result_table_noa_sorted_filtered_puts_all_tickers = 'noa/result_filtered_puts.csv'
    ...
    latest_ratio_column = ratio_columns[-2]  # Use the latest ratio column
    filtered_df_puts = final_df_sorted[
        (final_df_sorted['Total Calls'] > 30000) &
        (final_df_sorted[latest_ratio_column] > 0.3)
        ]
    # Format the numbers for output
    filtered_df_puts = filtered_df_puts.applymap(format_numbers)

    # Convert 'Expiration Date' to string in the '%m-%d' format
    filtered_df_puts['Expiration Date'] = filtered_df_puts['Expiration Date'].dt.strftime('%m-%d')

    print('DF before filter out of last call column with zeros')
    print(filtered_df_puts)

    # Identify the last column containing 'Call)'
    put_columns = [col for col in df.columns if "'Put')" in col]
    if not put_columns:
        print("No 'Put)' columns found.")
        return

    last_put_column = put_columns[-1]  # The last column containing 'Call)'

    filtered_df_puts[last_put_column] = filtered_df_puts[last_put_column].apply(clean_and_convert)

    print('ALL Put columns')
    print(put_columns)
    print('Last Call column')
    print(last_put_column[0])

    # Filter out rows with zero values in the last 'Call)' column
    filtered_df_puts = filtered_df_puts[filtered_df_puts[last_put_column] != 0]
    ...
    unique_tickers = filtered_df_puts['Stock Symbol'].unique()
    ...
    filtered_df_puts.to_csv(result_table_noa_sorted_filtered_puts_all_tickers, index=False)
    pd.Series(unique_tickers).to_csv(result_table_noa_sorted_filtered_puts_all_tickers, index=False)

    return unique_tickers_list, filtered_df_puts